        ...
        <div className="order-2 flex min-h-0 flex-col md:h-[560px] md:overflow-hidden">
          ...
          {t.home.whyUs.items.map((point, i) => (
            <ScrollReveal key={point.title} delay={i * 90} variant="right" className="flex w-full min-w-0 md:flex-1">
              <div
                className={`group relative z-0 flex w-full min-w-0 flex-1 items-start gap-6 border-t border-border px-8 py-8 transition-colors duration-500 md:px-10 md:py-8 ${i === 0 ? 'border-t-0' : ''}`}
              >
                <div
